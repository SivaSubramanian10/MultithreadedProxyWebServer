    ...
    #define SHUT_RDWR SD_BOTH  // Define SHUT_RDWR for Windows
// #else
    // #include <sys/socket.h>  // POSIX systems, includes SHUT_RDWR
#endif
...
#define CROSS_PLATFORM_SOCKET_H
    ...
    #include <ws2tcpip.h>     // Additional headers for socket API
    // #pragma comment(lib, "ws2_32.lib")  // Link with Winsock library

    // Windows-specific definitions
    typedef SOCKET socket_t;  // Alias for socket type in Windows
    #define close_socket closesocket  // Alias for close function in Windows

    // Initialization and cleanup for Windows sockets
    inline int init_sockets() {
        WSADATA wsaData;
        return WSAStartup(MAKEWORD(2, 2), &wsaData);
    }

    inline int cleanup_sockets() {
        return WSACleanup();
    }

#else
    // POSIX-specific headers and definitionsx`
    ...
    typedef int socket_t;  // Alias for socket type in Linux/macOS
    #define close_socket close  // Alias for close function in POSIX systems

    // No special socket initialization/cleanup needed on Unix-like systems
    inline int init_sockets() {
...
int handle_request(int clientSocket, ParsedRequest *request, char *tempReq)
{
	...
	while(bytes_send > 0)
	{
		...
		for(unsigned long long i=0;i<bytes_send/sizeof(char);i++){
    ...
    if(head!=NULL){
        ...
        while (site!=NULL)
        {
            if(!strcmp(site->url,url)){
				printf("LRU Time Track Before : %lld", (long long)site->lru_time_track);
                printf("\nurl found\n");
				// Updating the time_track
				site->lru_time_track = time(NULL);
				printf("LRU Time Track After : %lld", (long long)site->lru_time_track);
				break;
            }
            site=site->next;
        }       
